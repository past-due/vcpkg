...
namespace vcpkg::Commands::Hash
{
    ...
    struct BCryptAlgorithmHandle : Util::ResourceBase
    {
        ...
        {
            if (this->handle) BCryptCloseAlgorithmProvider(this->handle, 0);
        }
    };
    ...
    std::string get_file_hash(const VcpkgPaths&, const fs::path& path, const std::string& hash_type)
    {
        ...
        const std::wstring w_algorirthn = Strings::to_utf16(Strings::ascii_to_uppercase(hash_type));

        NTSTATUS error_code = BCryptOpenAlgorithmProvider(&algorithm_handle.handle, w_algorirthn.c_str(), nullptr, 0);
        ...
        BCryptAlgorithmHandle hash_handle{};
